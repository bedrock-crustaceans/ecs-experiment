...
impl<R: Resource> ResourceSingleton<R> {
    pub fn acquire_read(&self) -> EcsResult<*const R> {
    ...
    pub fn acquire_write(&self) -> EcsResult<*mut R> {
...
impl Resources {
    ...
    pub fn get<R: Resource>(&self) -> EcsResult<&R> {
    ...
    pub fn get_mut<R: Resource>(&self) -> EcsResult<&mut R> {
...
impl<R: Resource> Deref for ResMut<R> {
    ...
    fn deref(&self) -> &Self::Target {
        // The drop implementation expects a write lock, therefore use one here as well.
        // Systems using a ResMut will be ran in isolation anyways.
